...
func (L *State) getFreeIndex() (index uint, ok bool) {
  ...
  if freelen > 0 {
    ...
    L.freeIndices = L.freeIndices[0 : len(L.freeIndices)-1] //'pop' index from list
    return i, true
  }
  return 0, false
}
...
func (L *State) PushGoFunctionAsCFunction(f GoFunction) {
  fid := L.register(f)
  C.clua_pushgofunctionascfunction(L.s, C.uint(fid))
}
